...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # Terminal mein check karne ke liye (Niche VS Code mein dikhega)
            print(f"Logged in user: {username}")
            print(f"Expected Shopkeeper: {os.getenv('SHOPKEEPER_USER')}")

            # 1. Sabse pehle Shopkeeper check karein (Sahi tareeka)
            if str(username).strip() == str(os.getenv("SHOPKEEPER_USER")).strip():
                print("Redirecting to Shopkeeper Dashboard...")
                return redirect("shopkeeper_dashboard")
            
            # 2. Phir Admin check karein
            elif str(username).strip() == str(os.getenv("ADMIN_USER")).strip():
                print("Redirecting to Admin Panel...")
                return redirect("/admin/")
            
            # 3. Agar koi match nahi hua toh Customer
            else:
                print("No match found, redirecting to Customer Homepage...")
...
def verify_otp(request):
    if request.method == "POST":
        ...
        if str(entered_otp) == str(data["otp"]):
            try:
                # 1. Pehle check karenge ki user pehle se toh nahi bana
                if User.objects.filter(username=data["username"]).exists():
                    messages.error(request, "Username already taken.")
                    return redirect("signup")

                # 2. User Create karenge 
                new_user = User(
                ...
                # Terminal check karenge, wahan asal error dikhega
